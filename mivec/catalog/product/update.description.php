...
//$cat = Mage::getModel('catalog/category')->load(90);
							...
							foreach($attributes as $attribute ){
                                          if($attribute->getDefaultValue()=='use_as_header'){
										   ...
										   if($attribute->getFrontend()->getValue($_product)=="microphone"){
											   ...
											   // echo   $_product->getId();
											    $desc = $_product->getDescription();
												$descUp = '<p>Har mikrofonen slutat fungera på din iPhone? Denna funktion kan påverka både telefonsamtal, röststyrning och inspelningar. Vänta inte med att få din mikrofon reparerad!</p>';
												$update->setDescription($descUp);
												 try {
													 $update->save();
													 echo "<pre>".$_product->getId(). " successfully</pre>";
